             ...
             <div class="bg-white">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($withdrawals as $withdraws)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $withdraws->user->name ?? 'Unknown User'  }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ number_format($withdraws->amount,2) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ ucfirst($withdraws->payment_method) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-green-600 font-semibold">
                                    @if($withdraws->status == 'Pending')
                                        <a href="{{route('withdrawal.approve',$withdraws->id)}}" >Approve</a>
                                        <a href="{{route('withdrawal.reject',$withdraws->id)}}" class="text-red-500">Reject</a>
                                    @else
                                        {{ $withdraws->status }}
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $withdraws->created_at->format('d M Y') }}</td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="2" class="px-6 py-4 text-center text-sm text-gray-500">No users found</td>
